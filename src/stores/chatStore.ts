...
interface FloatingChatActions {
    ...
    updateChatWindowPosition: ( conversationId: string, position: { x: number; y: number } ) => void;
    updateChatWindowSize: ( conversationId: string, size: { width: number; height: number } ) => void;
...
const defaultWindowPosition = { x: window.innerWidth - 400, y: window.innerHeight - 650 };
