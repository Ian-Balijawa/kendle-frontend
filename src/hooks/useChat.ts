...
export function useInfiniteMessages( conversationId: string, params: GetMessagesParams = {} ) {
    ...
    return useMutation( {
        ...
        onError: ( _err, _newConversation, context ) => {
            ...
            if ( context?.optimisticConversation ) {
                queryClient.setQueryData( chatKeys.conversations(), ( old: Conversation[] | undefined ) => {
                    if ( !old ) return old;
                    return old.filter( conv => conv.id !== context.optimisticConversation.id );
                } );
            }
        },
        onSuccess: ( newConversation, _variables, context ) => {
            ...
            if ( context?.optimisticConversation ) {
                queryClient.setQueryData( chatKeys.conversations(), ( old: Conversation[] | undefined ) => {
                    if ( !old ) return old;
                    return old.map( conv =>
                        conv.id === context.optimisticConversation.id ? newConversation : conv
                    );
                } );
            }
...
export function useUpdateConversation() {
    ...
    return useMutation( {
        ...
        onMutate: async ( { id, data } ) => {
            ...
            const previousConversation = queryClient.getQueryData( chatKeys.conversation( id ) );

            // Optimistically update the single conversation
            queryClient.setQueryData( chatKeys.conversation( id ), ( old: Conversation | undefined ) => {
                if ( !old ) return old;
                return {
                    ...old,
                    ...data,
        ...
        onError: ( _err, { id }, context ) => {
            ...
            if ( context?.previousConversation ) {
                queryClient.setQueryData( chatKeys.conversation( id ), context.previousConversation );
            }
            queryClient.invalidateQueries( { queryKey: chatKeys.conversations() } );
        },
...
export function useSendMessage() {
    ...
    return useMutation( {
        ...
        onMutate: async ( { conversationId, data } ) => {
            // Cancel any outgoing refetches
            await queryClient.cancelQueries( { queryKey: chatKeys.messagesList( conversationId ) } );
            await queryClient.cancelQueries( { queryKey: chatKeys.conversations() } );
...
export function useDeleteMessage() {
    ...
    return useMutation( {
        ...
        onMutate: async ( id ) => {
            ...
            const messageQueries = queryClient.getQueriesData( { queryKey: chatKeys.all } );
        ...
        onSuccess: ( _, _id, context ) => {
            ...
            if ( context?.conversationId ) {
                queryClient.invalidateQueries( { queryKey: chatKeys.messages( context.conversationId ) } );
                queryClient.invalidateQueries( { queryKey: chatKeys.conversations() } );
            }
        },
    } );
    ...
    return useMutation( {
        ...
        onSuccess: ( conversation ) => {
            ...
            queryClient.setQueryData( chatKeys.conversations(), ( old: Conversation[] | undefined ) => {
                ...
                if ( exists ) {
                    return old.map( conv => conv.id === conversation.id ? conversation : conv );
                }
