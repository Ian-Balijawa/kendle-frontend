...
export const followKeys = {
    ...
    followersForUser: ( userId: string ) => [...followKeys.followers(), userId] as const,
    following: () => [...followKeys.all, "following"] as const,
    followingForUser: ( userId: string ) => [...followKeys.following(), userId] as const,
...
export function useFollowUser() {
    ...
    return useMutation( {
        ...
        onMutate: async ( targetUserId ) => {
            ...
            await queryClient.cancelQueries( { queryKey: followKeys.statusForUser( targetUserId ) } );

            // Snapshot the previous value
            const previousStatus = queryClient.getQueryData( followKeys.statusForUser( targetUserId ) );
            ...
            const userProfile = queryClient.getQueryData( ["users", "profile"] );
            if ( userProfile ) {
        ...
        onError: ( error, targetUserId, context ) => {
            ...
            if ( context?.previousStatus ) {
                queryClient.setQueryData( followKeys.statusForUser( targetUserId ), context.previousStatus );
            }
        ...
        onSuccess: ( data, targetUserId ) => {
            ...
            queryClient.invalidateQueries( { queryKey: followKeys.statusForUser( targetUserId ) } );
            queryClient.invalidateQueries( { queryKey: followKeys.followingForUser( user?.id || "" ) } );
            queryClient.invalidateQueries( { queryKey: followKeys.followersForUser( targetUserId ) } );
...
export function useToggleFollow() {
    ...
    const toggleFollow = async ( targetUserId: string, currentFollowStatus?: boolean ) => {
