...
import { Status, StatusesResponse, StatusReply, StatusRepliesResponse } from "../types";
...
export const statusKeys = {
    ...
    replies: (statusId: string) => [...statusKeys.all, "replies", statusId] as const,
    analytics: (statusId: string) => [...statusKeys.all, "analytics", statusId] as const,
};
    ...
    return useMutation({
        ...
        onMutate: async (newStatus) => {
            ...
            const optimisticStatus: Status = {
                ...
                allowReplies: newStatus.allowReplies ?? true,
                allowReactions: newStatus.allowReactions ?? true,
                allowShares: newStatus.allowShares ?? true,
                allowScreenshots: newStatus.allowScreenshots ?? true,
                ...
                expiresAt: new Date(Date.now() + (newStatus.expirationHours ?? 24) * 60 * 60 * 1000).toISOString(),
                isExpired: false,
                author: user
                    ? {
                        ...user,
            ...
            queryClient.setQueriesData({ queryKey: statusKeys.lists() }, (old: any) => {
                if (!old) return old;
...
export function useUpdateStatus() {
    return useMutation({
        ...
        onMutate: async ({ id, data }) => {
            ...
            queryClient.setQueryData(statusKeys.detail(id), (old: Status | undefined) => {
                if (!old) return old;
                return {
                    ...old,
                    ...data,
        ...
        onMutate: async ({ id, data }) => {
            ...
            queryClient.setQueriesData(
                ...
                (old: any) => {
                    ...
                    return {
                        ...old,
                        pages: old.pages.map((page: StatusRepliesResponse, index: number) => {
                            if (index === 0) {
                                return {
                                    ...page,
                                ...
                                if (index === 0) {
                                    return {
                                        ...
                                        replies: page.replies.filter(
                                            (reply: StatusReply) => reply.id !== context.optimisticReply.id,
                                        ),
                                        total: Math.max(0, page.total - 1),
                                    };
                                }
